...
workflow.onComplete {
    ...
    try {
        if (workflow.success) {
            ...
            if (mqc_report.getClass() == ArrayList) {
                log.warn "[nf-core/cnvcall] Found multiple reports from process 'multiqc', will use only one"
                mqc_report = mqc_report[0]
            }
        }
    } catch (all) {
        log.warn "[nf-core/cnvcall] Could not attach MultiQC report to summary email"
    }
    ...
    if (email_address) {
        try {
          ...
          log.info "[nf-core/cnvcall] Sent summary e-mail to $email_address (sendmail)"
        } catch (all) {
          // Catch failures and try with plaintext
          [ 'mail', '-s', subject, email_address ].execute() << email_txt
          log.info "[nf-core/cnvcall] Sent summary e-mail to $email_address (mail)"
        }
    }
